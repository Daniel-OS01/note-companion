...
async function processImageWithGPT4one(
  ...
  try {
    console.log("Processing image with models/gemini-2.5-pro for OCR...");
    const openai = createOpenAI({ apiKey: process.env.OPENAI_API_KEY });
    console.log(`Processing OCR for image: ${imageUrl}`);
    const { object, usage } = await generateObject({
      model: openai("models/gemini-2.5-pro"),
    ...
    console.log(
      `models/gemini-2.5-pro OCR extracted ${textContent.length} chars, used approx ${tokensUsed} tokens`
    );
    return { textContent, tokensUsed };
  } catch (error) {
    console.error("Error processing image with models/gemini-2.5-pro OCR:", error);
    return {
